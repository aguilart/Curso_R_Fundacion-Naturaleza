...
es recomendable que ejcutes el código anterior en la **consola** (ventana inferior izquierda).
...
Existen varios métodos. Es importante tener en cuenta ciertos lineamientos generales:

*1. Cuál tipo de archivo quiero importar.
*2. Cuál método es el más apropiado.
*3. Cuál es mi finalidad con el análisis de los datos.

Metodo 1. Importarlos usando la interfaz gráfica del usuario.

Para ello sigue los siguientes pasos

- Presiona click en "file" 
- Selcciona "Import data set"
- seleciona el tipo de archivo, en este ejemplo " From excel"
- selecciona "browse" en la esquina superior derecha para encontrar el archivo
- Verifica que las variables han sido identificadas correctamente
- Presiona "import" en la esquina inferioir derecha
- !listo

- Este metodo puede ser abreviado utilizando el entorno "Environment"

```{r}
#metodo1<-datos
```

Método 2. Importarlo por medio de la funcion "seleccionar archivo"

- Define y asigna el nombre a tu base de datos "método2"
- selecciona la funcion read.csv
- ejecuta la función "file.choose"
- la flecha verde superior derecha en el "chunk"
- Sigue la interfaz gráfica del usuario para encontrar tu archivo. Recuerda que en este caso debe ser un .csv
- Listo.

```{r}
metodo2 <- read.csv(file.choose())
```

Método 3. Importarlo por medio de comando (Recomendado)

- Define y asigna el nombre a tu base de datos
- selecciona la funcion read.csv
- Escribe la ruta del archivo de interes, recuerda incorporar las comillas "", "C:\Users\SGM\Desktop"
- Recuerda cambiar la direccion de la pleca hacia la derecha "/", "C:/Users/SGM/Desktop"
- Incluye junto con el nombre del archivo, la extension, en este caso .csv
- Verifica los espacios, y si en los nombres de la ruta hay espacios mantenlos, de lo contrario eliminalos.
- Presiona la flecha verde superior derecha en el "chunk"
- Listo.

```{r}
metodo3<-read.csv("C:/Users/SGM/Desktop/datos.csv")
metodo3
```
...
**Sepal.Length** La longitud de los sépalos en centímetros
**Sepal.Width** El ancho de los los sépalos en centímetros
**Petal.Length** La longitud de los pétalos en centímetros
**Petal.Width** El ancho de los pétalos en centímetros
**Species** Las especies estudiadas

![Fuente: ICHI.PRO](images\Jornada 3\iris_flores.png){width="600" height="200"} 
...
En técnicas mas avanzadas (no cubiertas en este curso) hay métodos que pueden corregir e incorporar conjuntos de observaciones que no cumplen con el supuesto de independencia como los Modelos Generalizados Lineales Mixtos o Generalized Linear Mixed Models en inglés (GLMM)

#### Histograma

El histograma de frecuencias es una alterniva excelente para observar la distribucion de variables continuas. Presenta una idea rapida de si los datos se ajustan a una distribucion normal y la presencia de valores extremos. Construyamos un histograma para nuestras variables númericas continuas.

```{r}
#png("figura 1", units="in", width=10, height=7, res=300)
histograma1<-ggplot(data=iris, aes(Petal.Length))+
...
```{r}
 # # ejemplovio<-ggplot(data=iris, aes(x=Species, y=Sepal.Length, fill=Species))+
 #   geom_violin()+
 #    coord_flip()+
...
![Caja: FUENTE WIKIPEDIA](images\Jornada 3\Diagrama_de_caja.jpg){width="300" height="400"} 
...
El gráfico de cajas puede ayudarnos aobservar la distribución de las variables y su normalidad. Para este caso
```{r}
...
El método alternativo es ocupar la función que viene incorporada por defecto en R y generar un conjunto de gráficos con la misma informacion
...
Existen muchas formas de presentar estos resultados. Esta es solo una opción.
...
El modelo se adhiere a los supuestos de linearidad (1), carencia de valores atipicos (4) independencia (5) y homogenindad de pendientes (6), pero no cumple lo#s supuestos de normalidad en los residuos (2) y homocedasticidad (3). Por lo tanto se debe ser cuidadoso al interpretar el modelo si es que se acepta. Escencialmente podemos mencionar que existen diferencias en el tamaño de los sépalos entre las especies, cuando se controla el efecto del ancho del sépalo.
